...
SCIENTIFIC HONESTY:
    ...
    - No proxy labels are used; only explicitly verified class mappings are accepted
...
def stage1_acquisition() -> Dict:
    ...
    Stage 1 only verifies already-present raw recordings.
    No internet download is performed. This keeps the pipeline
    deterministic and prevents it from hanging on network access.
    """
    log.info("=" * 70)
    ...
    # 1a. CMU Arctic — existing files only
...
def stage4_splits(processed_index: Dict[str, List[Path]]) -> Tuple[Dict, Dict]:
    """
    Assign leakage-free train/val/test splits.

    Clean speech:
        fixed speaker-disjoint split from CMU_SPEAKERS.

    Noise:
        hash-grouped split. Identical audio content (same MD5) is treated
        as one recording group and can belong to only ONE of train/val/test.
        The assignment is deterministic and approximately 70/15/15.

    This protects against duplicate/identical recordings appearing under
    different filenames or folders.
    """
    log.info("=" * 70)
    log.info("STAGE 4 — TRAIN / VAL / TEST SPLITS (Leakage-Free)")
    log.info("=" * 70)

    # ── Clean speech: speaker-disjoint ────────────────────────────────────────
    ...
    for split_name in ("train", "val", "test"):
        ...
        log.info(
            f"    {split_name:5s}: {len(clean_splits[split_name]):4d} utterances"
            f"  speakers={speakers_in}"
        )

    # ── Noise: globally hash-grouped split ────────────────────────────────────
    # Build hash -> all paths first. This means if the exact same audio exists
    # in multiple classes/folders, it is still assigned to one split only.
    hash_groups: Dict[str, List[Path]] = defaultdict(list)

    for cls in ALL_NOISE_CLASSES:
        for path in processed_index.get(f"noise/{cls}", []):
            try:
                file_hash = md5_hash(path)
                hash_groups[file_hash].append(path)
            except Exception as e:
                log.warning(f"  [HASH] Could not hash {path.name}: {e}")

    # Deterministic assignment of each unique hash group.
    # A stable hash bucket gives approximately 70/15/15 without randomness.
    ...
    for file_hash in sorted(hash_groups):
        bucket = int(file_hash[:8], 16) / 0xFFFFFFFF
        ...
        hash_to_split[file_hash] = assigned_split

        # Put every path with this exact content into the same split.
        for path in hash_groups[file_hash]:
            rel = path.relative_to(PROCESSED_ROOT)
            parts = rel.parts

            if len(parts) >= 2 and parts[0] == "noise":
                cls = parts[1]
                if cls in noise_splits:
                    noise_splits[cls][assigned_split].append(path)

    # Deduplicate paths defensively and sort for deterministic Stage 5 input.
    for cls in ALL_NOISE_CLASSES:
        for split_name in ("train", "val", "test"):
            unique_paths = sorted(set(noise_splits[cls][split_name]))
            noise_splits[cls][split_name] = unique_paths

    log.info("  Noise split (hash-grouped, approximately 70/15/15, seed-independent):")
    for cls in ALL_NOISE_CLASSES:
        sp = noise_splits[cls]
        ...
        log.info(
            f"    {cls:20s}: train={tr:4d}  val={va:4d}  "
            f"test={te:4d}  {status_str}"
        )

    # Explicit in-memory leakage assertion before Stage 5.
    split_hashes = {}
    for split_name in ("train", "val", "test"):
        split_hashes[split_name] = set()
        for cls in ALL_NOISE_CLASSES:
            for path in noise_splits[cls][split_name]:
                try:
                    split_hashes[split_name].add(md5_hash(path))
                except Exception:
                    pass

    tv = split_hashes["train"] & split_hashes["val"]
    tt = split_hashes["train"] & split_hashes["test"]
    vt = split_hashes["val"] & split_hashes["test"]

    if tv or tt or vt:
        raise RuntimeError(
            "Stage 4 hash-grouping failed: noise hashes still overlap "
            f"(train∩val={len(tv)}, train∩test={len(tt)}, val∩test={len(vt)})"
        )

    log.info("  [PASS] Stage 4 pre-check: zero noise-hash overlap across splits")
            ...
            for noise_path in sampled_noise:
                ...
                for clean_path in clean_sample:
                    ...
                    for snr_db in SNR_LEVELS:
                        ...
                        seed_key = f"{clean_path}|{noise_path}|{snr_db}".encode("utf-8")
                        seed_int = int(hashlib.md5(seed_key).hexdigest()[:8], 16) & 0x7FFFFFFF
